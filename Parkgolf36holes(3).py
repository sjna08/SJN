...
def app():
    ...
    for hole in holes:
        ...
        for player in players:
           scorecard.loc[player, hole] = st.number_input(f'{player} {hole} 점수', min_value=0, max_value=10, value=0, step=1, key=f'{player}_{hole}')
    ...
    if st.button('제출'):
        ...
        summary['B'] = scorecard.iloc[:, 9:18].sum(axis=
