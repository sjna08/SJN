...
def app():
    ...
    session_state = SessionState.get(scorecard=scorecard)

    # 사용자에게 입력 받기
    for hole in holes:
        st.subheader(hole)
        for player in players:
            session_state.scorecard.loc[player, hole] = st.number_input(f'{player} {hole} 점수', min_value=0, value=0, key=f'{player}_{hole}')

    # 입력 받은 정보를 표시하고 다운로드 받기
    if st.button('제출'):
        summary = pd.DataFrame(index=players, columns=['TTL','A','B', 'A_Dif','B_Dif','C','D', 'C_Dif','D_Dif','TTL_Dif'])
        summary['A'] = session_state.scorecard.iloc[:, :9].sum(axis=1)
        summary['B'] = session_state.scorecard.iloc[:, 9:18].sum(axis=1)
        summary['C'] = session_state.scorecard.iloc[:, 18:27].sum(axis=1)
        summary['D'] = session_state.scorecard.iloc[:, 27:].sum(axis=1)
        summary['TTL'] = summary[['A','B','C','D']].sum(axis=1)
        ...
        st.write(summary)
        st.write(session_state.scorecard.transpose())

        # 스코어카드와 요약을 합치기
        full_scorecard = pd.concat([summary, session_state.scorecard], axis=1)
