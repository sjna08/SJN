...
def app():
    ...
    if 'scorecard' not in st.session_state:
        st.session_state['scorecard'] = pd.DataFrame(index=players, columns=holes).astype(int)

    scorecard = st.session_state['scorecard']

    # 홀 선택
    hole_selection = st.sidebar.radio('홀 선택', ['A홀', 'B홀', 'C홀', 'D홀', '전체'])

    if hole_selection == 'A홀':
        selected_holes = holes[:9]
    elif hole_selection == 'B홀':
        selected_holes = holes[9:18]
    elif hole_selection == 'C홀':
        selected_holes = holes[18:27]
    elif hole_selection == 'D홀':
    ...
    if st.button('제출'):
        ...
        summary['TTL'] = summary.sum(axis=1)
